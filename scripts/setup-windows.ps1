...
if (Test-Path $EnvFile) {
    ...
    $lines = @(
        ...
        '# 5174 is the studio app and 5175 the guest dev server. Leaving them',
        '# out makes the studio app fail its login with "could not reach the',
        '# server", which reads like the API is down when it is answering fine',
        '# and refusing the origin.',
        "CORS_ORIGINS_RAW=$Hostname,http://localhost:8000,http://localhost:5174,http://localhost:5175,file://,null",
        '',
